...
[handles.answer handles.input_socketconex] = conex(handles.computeripconex,handles.serialserversocketport,msg);
...
if isempty(errorconex)
    ...
    [handles.answer handles.input_socketconex] = conex(handles.computeripconex,handles.serialserversocketport,msg,handles.input_socketconex);

    isnotref = char(handles.answer);

    if ~isequal(isnotref(4:7),'0000')
        str=strcat('The following errors occured ',isnotref(4:7),'.');
        errordlg(str);
    end
    ...
    if isequal(isnotref(8),'0') || isequal(isnotref(8:9),'10') % Homing
        ...
        if ~isequal(ishomed(4:7),'0000')
            str=strcat('the following errors occured ',ishomed(4:7),'.');
            errordlg(str);
        end
    end

    %%%% Move the linear activator to the position 2mm, where Autocorrelation shall be observed.

    msg = {['SENDRCV ' handles.termchar ' ' '1PA2'],['SENDRCV ' handles.termchar ' ' '1TS']}; % Creates a command to move absolute 2 mm
    [handles.answer handles.input_socketconex] = conex(handles.computeripconex,handles.serialserversocketport,msg,handles.input_socketconex);
    ...
    if get(handles.operation,'Value') == 2 % If Operation mode is displacement (relative move)
        ...
        if get(handles.direction,'Value') == 1 % Get direction (Forward)
            ...
            if str2double(posumnew) > handles.upperlimit*1000
                ...
                i=1;
                %Tests whether the linear activator reached the home position. If not wait and try again.
                while (i<=100 && ~isequal(ispositionreached(8:9),'33'))
